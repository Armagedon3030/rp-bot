...
count_initial_slot = list(map(int, list("0" * 15)))
...
async def on_ready():
    ...
    for guild in bot.guilds:
        for member in guild.members:
            if cursor.execute(f"SELECT id FROM users WHERE id = {member.id} AND guild = {guild.id}").fetchone() is None and member.bot == False:
                cursor.execute(f"INSERT INTO users VALUES ('{member}', {guild.id}, {member.id}, 5, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", initial_inventory)
...
`{bot.command_prefix}edit_slot [уровень инвентаря(1-3)]` - добавление слотов в инвентарь
...
```"""
        ...
        embed.description = f"""Данной команды не существует или команда максимально проста, что не имеет никакой дополнительной инвормации
...
@bot.command(aliases=["del_inv", "del_inventory", "del"])
...
async def __admin(ctx, user: discord.Member, *, command):
    ...
    if command[0] in ["inv", "inventory"]:
        ...
        await __add_inv(ctx=ctx, index=command[1], count=command[2], thing=command[3], change=True, user=user)
